...
return [
    ...
    [
        ...
        'tekst' => [
            'nl' => '🔧 <b>Reserve inzetten — maximum-override + overleeft re-import</b> — een handmatig ingezette reserve werd bij opnieuw importeren teruggezet en viel uit de loting; nu blijven zulke reserves staan (status én reserve beschermd). Ook honoreert het inzetten nu het handmatig verhoogde maximum (max in loting), zodat je een reserve kunt inzetten tot dat hogere maximum i.p.v. te stranden op het aantal KNSB-inschrijvingen. Het importscherm blijft daarna niet meer onterecht op "onopgeslagen wijzigingen" staan.',
            'en' => '🔧 <b>Deploying reserves — maximum override + survives re-import</b> — a manually deployed reserve was reset on re-import and dropped from the draw; such reserves now stay in place (both status and reserve protected). Deploying also now honours a manually raised maximum (max in draw), so you can deploy a reserve up to that higher maximum instead of being blocked at the number of KNSB entries. The import screen no longer wrongly reports "unsaved changes" afterwards.',
            'de' => '🔧 <b>Reserve einsetzen — Maximum-Override + übersteht Re-Import</b> — eine manuell eingesetzte Reserve wurde beim erneuten Import zurückgesetzt und fiel aus der Auslosung; solche Reserven bleiben jetzt bestehen (Status und Reserve geschützt). Das Einsetzen berücksichtigt jetzt auch ein manuell erhöhtes Maximum (Max in Auslosung), sodass du eine Reserve bis zu diesem höheren Maximum einsetzen kannst statt an der Anzahl der KNSB-Anmeldungen zu scheitern. Der Import-Bildschirm meldet danach nicht mehr fälschlich "ungespeicherte Änderungen".',
            'fr' => '🔧 <b>Engager des réserves — maximum forcé + survit au ré-import</b> — une réserve engagée manuellement était réinitialisée lors d\'un ré-import et sortait du tirage ; ces réserves restent désormais en place (statut et réserve protégés). L\'engagement respecte aussi maintenant un maximum relevé manuellement (max au tirage), de sorte que vous pouvez engager une réserve jusqu\'à ce maximum supérieur au lieu d\'être bloqué au nombre d\'inscriptions KNSB. L\'écran d\'import n\'indique plus à tort « modifications non enregistrées » ensuite.',
        ],
    ],
    [
        'versie' => 'H1243.10.08', 'datum' => '26-08-2026',
