...
return [
    ...
    [
        ...
        'tekst' => [
            'nl' => '🕶 <b>Publiek anoniem</b> — een rijder kan er nu zelf voor kiezen om niet met naam in de openbare pagina\'s te staan; bij jeugd regelt de ouder/verzorger dat. In plaats van naam, vereniging en woonplaats verschijnt dan <b>“Anoniem”</b>, en ook het <b>startnummer</b> wordt verborgen (dat is in Nederland meerjarig vast, dus anders zou je daarmee alsnog herkenbaar zijn). Eén uitzondering: van de <b>dag vóór tot en met de dag ná de wedstrijd</b> zijn de naam én het startnummer wél zichtbaar op de startlijst en de uitslag van díe wedstrijd — dan hebben rijders die gegevens nodig om te zien in welke rit ze zitten. Daarbuiten, én in het blijvende uitslagen-archief en het serie-klassement, blijft het “Anoniem”, en de rijder is ook niet op naam te vinden via de zoekfunctie. Instellen doe je zelf via <i>Mijn InlineComp</i>, of je vraagt het aan de organisatie. Wil je dat iemand je tóch kan volgen — bijvoorbeeld je ouder of coach — dan deel je jouw persoonlijke volg-ID met die persoon; daarmee ziet hij of zij jouw naam wél.',
            'en' => '🕶 <b>Publicly anonymous</b> — a skater can now choose not to appear by name on the public pages; for youth, the parent/guardian arranges this. Instead of name, club and place of residence it shows <b>“Anonymous”</b>, and the <b>start number</b> is hidden too (in the Netherlands it stays the same for years, so otherwise you\'d still be recognisable). One exception: from the <b>day before through the day after the competition</b> the name and start number are visible on the start list and results of that competition — that\'s when skaters need this to see which heat they\'re in. Outside that window, and in the permanent results archive and the series standings, it stays “Anonymous”, and the skater can\'t be found by name via the search either. You set it yourself in <i>My InlineComp</i>, or ask the organisation. If you do want someone to be able to follow you — your parent or coach, say — you share your personal follow-ID with that person; that lets them see your name.',
            'de' => '🕶 <b>Öffentlich anonym</b> — ein Fahrer kann jetzt selbst wählen, nicht mit Namen auf den öffentlichen Seiten zu stehen; bei Jugendlichen regeln das die Eltern bzw. Erziehungsberechtigten. Statt Name, Verein und Wohnort erscheint dann <b>„Anonym“</b>, und auch die <b>Startnummer</b> wird verborgen (sie bleibt in den Niederlanden über Jahre gleich, sonst wärst du weiterhin erkennbar). Eine Ausnahme: vom <b>Tag vor bis zum Tag nach dem Wettkampf</b> sind Name und Startnummer auf der Startliste und im Ergebnis dieses Wettkampfs sichtbar — dann brauchen die Fahrer diese Angaben, um zu sehen, in welchem Lauf sie sind. Außerhalb dieses Zeitraums sowie im dauerhaften Ergebnisarchiv und im Serien-Klassement bleibt es „Anonym“, und der Fahrer ist auch nicht per Namen über die Suche zu finden. Du stellst es selbst in <i>Mein InlineComp</i> ein oder fragst die Organisation. Möchtest du, dass dich jemand trotzdem verfolgen kann — etwa dein Elternteil oder Trainer — teilst du deine persönliche Verfolgungs-ID mit dieser Person; damit sieht sie deinen Namen.',
            'fr' => '🕶 <b>Anonyme en public</b> — un patineur peut désormais choisir de ne pas apparaître avec son nom sur les pages publiques ; pour les mineurs, c\'est le parent/tuteur qui s\'en charge. Au lieu du nom, du club et du lieu de résidence, il s\'affiche <b>« Anonyme »</b>, et le <b>numéro de départ</b> est également masqué (aux Pays-Bas il reste identique pendant des années, sinon vous resteriez reconnaissable). Une exception : de la <b>veille au lendemain de la compétition</b>, le nom et le numéro de départ sont visibles sur la liste de départ et le résultat de cette compétition — c\'est le moment où les patineurs en ont besoin pour voir dans quelle série ils se trouvent. En dehors de cette période, ainsi que dans l\'archive permanente des résultats et le classement de série, cela reste « Anonyme », et le patineur n\'est pas non plus trouvable par son nom via la recherche. Vous le réglez vous-même dans <i>Mon InlineComp</i>, ou demandez à l\'organisation. Si vous voulez que quelqu\'un puisse quand même vous suivre — votre parent ou coach, par exemple — vous partagez votre ID de suivi personnel avec cette personne ; cela lui permet de voir votre nom.',
        ],
    ],
    [
    ...
    [
        'versie' => 'H2160.18.09', 'datum' => '18-09-2026', 'onderdelen' => ['admin'],
        'tekst' => [
            'nl' => '🔑 <b>Eigen rijder-ID onder de motorkap</b> — InlineComp koppelt rijders en resultaten nu intern aan een <b>eigen, intern ID</b> in plaats van aan het KNSB-relatie-/licentienummer. Voor het dagelijks gebruik verandert er niets; wél is het privacy-vriendelijker (het KNSB-nummer is niet langer de sleutel) en het maakt functies als \'publiek anoniem\' en het persoonlijke volg-ID mogelijk. Bovendien is het een stabiele, bron-onafhankelijke sleutel, waardoor koppelingen met andere systemen — zoals inschrijfmodules en resultaatverwerking van derden — eenvoudiger en robuuster worden. Bij het <b>anonimiseren</b> van een rijder — het definitieve AVG-wissen op verzoek, iets anders dan het omkeerbare \'publiek anoniem\' hierboven — wordt nu ook de licentie-koppeling gewist, zodat er alleen een intern kenmerk overblijft.',
            'en' => '🔑 <b>Own rider ID under the hood</b> — InlineComp now links riders and results internally to its <b>own, internal ID</b> instead of the KNSB relation/licence number. Nothing changes for day-to-day use; it is more privacy-friendly (the KNSB number is no longer the key) and it makes features like \'publicly anonymous\' and the personal follow-ID possible. It is also a stable, source-independent key, which makes links with other systems — such as registration modules and third-party results processing — simpler and more robust. When a rider is <b>anonymised</b> — the definitive erasure on request under data-protection law, distinct from the reversible \'publicly anonymous\' above — the licence link is now wiped too, leaving only an internal identifier.',
            'de' => '🔑 <b>Eigene Fahrer-ID unter der Haube</b> — InlineComp verknüpft Fahrer und Ergebnisse jetzt intern mit einer <b>eigenen, internen ID</b> statt mit der KNSB-Beziehungs-/Lizenznummer. Für den täglichen Gebrauch ändert sich nichts; es ist datenschutzfreundlicher (die KNSB-Nummer ist nicht mehr der Schlüssel) und ermöglicht Funktionen wie \'öffentlich anonym\' und die persönliche Verfolgungs-ID. Zudem ist es ein stabiler, quellenunabhängiger Schlüssel, wodurch Verknüpfungen mit anderen Systemen — etwa Anmeldemodulen und der Ergebnisverarbeitung Dritter — einfacher und robuster werden. Beim <b>Anonymisieren</b> eines Fahrers — dem endgültigen Löschen auf Wunsch nach Datenschutzrecht, etwas anderes als das umkehrbare \'öffentlich anonym\' oben — wird jetzt auch die Lizenzverknüpfung gelöscht, sodass nur ein internes Kennzeichen bleibt.',
            'fr' => '🔑 <b>Identifiant patineur propre, en coulisses</b> — InlineComp relie désormais les patineurs et les résultats en interne à un <b>identifiant propre et interne</b> au lieu du numéro de relation/licence KNSB. Rien ne change pour l\'usage quotidien ; c\'est plus respectueux de la vie privée (le numéro KNSB n\'est plus la clé) et cela rend possibles des fonctions comme « anonyme en public » et l\'ID de suivi personnel. C\'est en outre une clé stable et indépendante de la source, ce qui rend les liens avec d\'autres systèmes — comme les modules d\'inscription et le traitement des résultats par des tiers — plus simples et plus robustes. Lors de l\'<b>anonymisation</b> d\'un patineur — l\'effacement définitif sur demande au sens de la protection des données, distinct de l\'« anonyme en public » réversible ci-dessus — le lien de licence est désormais aussi effacé, ne laissant qu\'un identifiant interne.',
        ],
    ],
    [
    ...
    [
        ...
        'tekst' => [
            ...
            'en' => '👤 <b>Manage rider profiles</b> — in System → Riders you now see whether a rider has a <i>My InlineComp</i> profile. You generate a one-time sign-up link (with a pre-filled, uniqueness-checked username), can regenerate it if someone loses their PIN, and can delete a profile. The <b>Profiles</b> button lists all riders with a profile or a pending request in one list (with a status badge); type in the search box to filter within it. The underlying results are kept when deleting.',
            'de' => '👤 <b>Fahrerprofile verwalten</b> — unter System → Fahrer siehst du jetzt, ob ein Fahrer ein <i>Mein InlineComp</i>-Profil hat. Du erzeugst einen einmaligen Anmeldelink (mit vorausgefülltem, auf Eindeutigkeit geprüftem Benutzernamen), kannst ihn neu erzeugen, wenn jemand seine PIN vergessen hat, und ein Profil löschen. Mit der Schaltfläche <b>Profile</b> siehst du alle Fahrer mit einem Profil oder einer offenen Anfrage in einer Liste (mit Status-Badge); tippe im Suchfeld, um darin zu filtern. Die zugrunde liegenden Ergebnisse bleiben beim Löschen erhalten.',
            'fr' => '👤 <b>Gérer les profils des patineurs</b> — dans Système → Patineurs, vous voyez désormais si un patineur possède un profil <i>Mon InlineComp</i>. Vous générez un lien d\'inscription à usage unique (avec un nom d\'utilisateur pré-rempli et vérifié comme unique), pouvez le régénérer si quelqu\'un a oublié son code PIN, et supprimer un profil. Le bouton <b>Profils</b> affiche tous les patineurs ayant un profil ou une demande en attente dans une liste (avec badge de statut) ; tapez dans le champ de recherche pour filtrer à l\'intérieur. Les résultats sous-jacents sont conservés lors de la suppression.',
        ],
    ],
    [
        'versie' => 'H1983.10.09', 'datum' => '10-09-2026', 'onderdelen' => ['admin'],
