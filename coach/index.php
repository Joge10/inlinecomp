...
const T = {
    nl: {
        ...
        help_p_auto_html: 'De pagina ververst zichzelf elke 3 minuten zolang het tabblad zichtbaar is. Het tijdstip van de laatste verversing zie je rechtsboven (<b>🔄 HH:MM</b>). Direct verversen kan ook: trek de pagina <b>naar beneden</b> (pull-to-refresh) of dubbelklik op de blauwe kop.',
    ...
    en: {
        ...
        help_p_auto_html: 'The page refreshes itself every 3 minutes as long as the tab is visible. The time of the last refresh is shown in the top right (<b>🔄 HH:MM</b>). Refresh immediately also works: pull the page <b>downwards</b> (pull-to-refresh) or double-click the blue header.',
    ...
    de: {
        ...
        help_p_auto_html: 'Die Seite aktualisiert sich alle 3 Minuten, solange der Tab sichtbar ist. Die Zeit der letzten Aktualisierung wird oben rechts angezeigt (<b>🔄 HH:MM</b>). Sofort aktualisieren funktioniert auch: ziehe die Seite <b>nach unten</b> (pull-to-refresh) oder doppelklicke auf den blauen Header.',
    ...
    fr: {
        ...
        help_p_auto_html: 'La page se rafraîchit toutes les 3 minutes tant que l\'onglet est visible. L\'heure de la dernière actualisation est affichée en haut à droite (<b>🔄 HH:MM</b>). Actualiser immédiatement fonctionne aussi: tirez la page <b>vers le bas</b> (pull-to-refresh) ou double-cliquez sur l\'en-tête bleu.',
...
// behaalde punten wél, doorgestreept + note. Inline-styled (los van stylesheet).
...
async function toonRitDetail(el) {
    ...
    try {
        ...
        const pkMax    = heeftPK ? pkMaxRnd(heat.rijders) : 0;
        const rijen = heat.rijders.map(r => {
            ...
            return `<tr class="${isMij ? 'mijn' : ''}">
                ...
                ${heeftPK  ? `<td class="col-pk">${pkPuntCel(r, pkMax)}</td>` : ''}
...
function toonMeldingenOverzicht() {
    ...
    const items = _meldingLijst.map(m => {
        ...
        const linkHtml = m.link_url
            ? `<a href="${escFn(m.link_url)}" target="_blank" rel="noopener"
                   ...
                   🔗 <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escFn(_meldingTekst(m, 'link_tekst'))}</span>
                </a>`
            : '';
