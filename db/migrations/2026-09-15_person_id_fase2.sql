...
--
--  COLLATION: sommige kolommen (bv. push_sub_licenses.person_license) zijn met een
--  andere collation aangemaakt dan persons.license_key → een kale JOIN op `=` geeft
--  "#1267 Illegal mix of collations". Daarom forceren we in elke backfill-JOIN
--  expliciet `COLLATE utf8mb4_unicode_ci` op de kind-kolom (wint van IMPLICIT).
--
...
UPDATE `entries` c JOIN `persons` p ON p.`license_key` = c.`person_license` COLLATE utf8mb4_unicode_ci SET c.`person_id` = p.`person_id` WHERE c.`person_id` IS NULL;
...
UPDATE `heat_entries` c JOIN `persons` p ON p.`license_key` = c.`person_license` COLLATE utf8mb4_unicode_ci SET c.`person_id` = p.`person_id` WHERE c.`person_id` IS NULL;
...
UPDATE `transponders` c JOIN `persons` p ON p.`license_key` = c.`person_license` COLLATE utf8mb4_unicode_ci SET c.`person_id` = p.`person_id` WHERE c.`person_id` IS NULL;
...
UPDATE `competition_startnummers` c JOIN `persons` p ON p.`license_key` = c.`person_license` COLLATE utf8mb4_unicode_ci SET c.`person_id` = p.`person_id` WHERE c.`person_id` IS NULL;
...
UPDATE `uitslag_afstand` c JOIN `persons` p ON p.`license_key` = c.`person_license` COLLATE utf8mb4_unicode_ci SET c.`person_id` = p.`person_id` WHERE c.`person_id` IS NULL;
...
UPDATE `uitslag_klassement` c JOIN `persons` p ON p.`license_key` = c.`person_license` COLLATE utf8mb4_unicode_ci SET c.`person_id` = p.`person_id` WHERE c.`person_id` IS NULL;
...
UPDATE `coach_athletes` c JOIN `persons` p ON p.`license_key` = c.`person_license` COLLATE utf8mb4_unicode_ci SET c.`person_id` = p.`person_id` WHERE c.`person_id` IS NULL;
...
UPDATE `organisatie_transponders` c JOIN `persons` p ON p.`license_key` = c.`person_license` COLLATE utf8mb4_unicode_ci SET c.`person_id` = p.`person_id` WHERE c.`person_id` IS NULL;
...
UPDATE `push_sub_licenses` c JOIN `persons` p ON p.`license_key` = c.`person_license` COLLATE utf8mb4_unicode_ci SET c.`person_id` = p.`person_id` WHERE c.`person_id` IS NULL;
...
UPDATE `klassement_posities` c JOIN `persons` p ON p.`license_key` = c.`license_key` COLLATE utf8mb4_unicode_ci SET c.`person_id` = p.`person_id` WHERE c.`person_id` IS NULL;
...
UPDATE `rijder_profiel` c JOIN `persons` p ON p.`license_key` = c.`license_key` COLLATE utf8mb4_unicode_ci SET c.`person_id` = p.`person_id` WHERE c.`person_id` IS NULL;
...
UPDATE `rijder_profiel_aanvraag` c JOIN `persons` p ON p.`license_key` = c.`license_key` COLLATE utf8mb4_unicode_ci SET c.`person_id` = p.`person_id` WHERE c.`person_id` IS NULL;
